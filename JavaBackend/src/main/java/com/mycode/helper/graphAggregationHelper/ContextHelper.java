package com.mycode.helper.graphAggregationHelper;
...
public class ContextHelper extends GraphAggregationHelper {
    public ContextHelper(SKG graph, Vertex<TypeDeclaration<?>> topVertex, Set<Integer> clusterIds) {
    ...
    protected TypeDeclaration<?> classVertex2Declaration(Vertex<?> vertex) {
        ...
        graph.outgoingEdgesOf(vertex).stream()
                ...
                .filter(toReserveVertex -> {
                    ...
                    // 保留无关代码
                    if (Collections.disjoint(clusterIds, topVertex.getClusterIds())) {
                        return false;
                    }
                    return true;
                })
