...
public class TelegramBotMsgHandler {
    ...
    private String updateCommentStatus(String msgContent, boolean status) {
        ...
        return String.format("<b>操作成功！</b>\n" + "\n" + "评论ID：%d\n" + "状态：%s", commentId, status ? "公开" : "隐藏");
    }
    ...
    private String deleteComment(String msgContent) {
        ...
        return String.format("<b>删除成功！</b>\n" + "\n" + "评论ID：%d\n" + "该评论及其子评论已删除", commentId);
    }
    ...
    private String replyComment(String msgContent) {
        ...
        if (splitIndex2 != -1 && msgContent.length() > splitIndex2 + 1) {
            ...
            return String.format("<b>回复成功！</b>\n" + "\n" + "<b>您在<a href=\"%s\">《%s》</a>给 <b>%s</b> 的快捷回复：</b>\n" + "\n" + "<pre>%s</pre>\n" + "\n" + "<b>其他信息：</b>\n" + "评论ID：<code>%d</code>\n" + "IP：%s\n" + "时间：<u>%s</u>\n" + "邮箱：<code>%s</code>\n" + "状态：%s [<a href=\"%s\">管理评论</a>]\n", blogProperties.getView() + commentPageEnum.getPath(), commentPageEnum.getTitle(), parentComment.getNickname(), comment.getContent(), comment.getId(), comment.getIp(), simpleDateFormat.format(comment.getCreateTime()), comment.getEmail(), comment.getPublished() ? "公开" : "待审核", blogProperties.getCms() + "/blog/comment/list");
        }
        return "<b>命令格式有误！</b>\n\n" + HELP_MESSAGE;
    }
    ...
    private String newMoment(String content) {
        ...
        return String.format("<b>动态发布成功！\n</b>" + "\n" + "<pre>%s</pre>\n" + "\n" + "<b>其他信息：</b>\n" + "时间：<u>%s</u>\n" + "状态：%s [<a href=\"%s\">管理动态</a>]\n", content, simpleDateFormat.format(moment.getCreateTime()), moment.getPublished() ? "公开" : "隐藏", blogProperties.getCms() + "/blog/moment/list");
    }
    ...
    private void handleCommandException(Exception e) {
        String result = String.format("<b>异常错误：</b>\n" + "exception: %s\n" + "message: %s", e.getClass(), e.getMessage());
        sendResult(result);
    }
